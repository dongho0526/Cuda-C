...
cudaError_t histoSharedWithSyncCuda(unsigned int* hIn, unsigned int* hOut, int xSize, int ySize);
cudaError_t histoSharedMemBlockWithCuda(unsigned int* hIn, unsigned int* hOut, int xSize, int ySize);
...
{
	int j = threadIdx.x;
	for (int i = 0; i < size; i+=HISTOBINS) {
		a[j] += c[i];
	}
}
...
int main()
{
	...
	histoCudaSharedTableSync = new unsigned int[HISTOBINS];
	...
	verifyResult = verify(histoCTable, histoCudaSharedTableSync);
	if (verifyResult == 0) {
		fprintf(stderr, "Verify histoCudaSharedTableSync Failed \n");
	}
	else {
		fprintf(stderr, "Verify histoCudaSharedTableSync Successfule \n");
	}

	verifyResult = verify(histoCTable, histoCudaSharedMemBlock);
	if (verifyResult == 0) {
		fprintf(stderr, "Verify histoCudaSharedMemBlock Failed \n");
	}
	else {
		fprintf(stderr, "Verify histoCudaSharedMemBlock Successfule \n");
	}


	printf("index\t\tCTable\t\tCudaTable\tCudaTableAtomic\tCudaTableSharedSync\tCudaSharedBlockTable\n");
	for (int i = 0; i < HISTOBINS; i++) {
...
cudaError_t histoSharedWithSyncCuda(unsigned int* hIn, unsigned int* hOut, int xSize, int ySize)
{
	...
	timer.Start();    
	histoSharedSyncKernel << <numBlocks, numThreads >> > (dev_in, dev_out, xSize, ySize);
	cudaStatus = cudaDeviceSynchronize();
	timer.Stop();

	printf("With histoSharedSyncKernel Time  elapsed=%g ms\n", timer.Elapsed());
...
cudaError_t histoSharedMemBlockWithCuda(unsigned int* hIn, unsigned int* hOut, int xSize, int ySize)
{
	...
	timer.Start();
	histoSharedSyncBlockKernel << <numBlocks, numThreads >> > (dev_in, dev_histoBlock_out, xSize, ySize);
	histoAccm<< <1, HISTOBINS >> > (dev_histoBlock_out, dev_out, numBlocks.x * numBlocks.y* HISTOBINS);
	cudaStatus = cudaDeviceSynchronize();
	timer.Stop();
	printf("With histoSharedSyncBlockKernel  Time  elapsed=%g ms\n", timer.Elapsed());
