...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	unsigned char* outputConvPtr,* outputConvPtrX, * outputConvPtrY;				//Output Image
	float* filterPtrX, * filterPtrY;
	int xSize, ySize;
	...
	outputConvPtrX = new unsigned char[buffer_size];
	outputConvPtrY = new unsigned char[buffer_size];


	filterPtrX = new  float[MAX_FILTER_WIDTH * MAX_FILTER_HEIGHT * sizeof(float)];
	filterPtrY = new  float[MAX_FILTER_WIDTH * MAX_FILTER_HEIGHT * sizeof(float)];
	...
	*****************************************************/
	...
	for (loopCount = 0; loopCount < 100; loopCount++) {
		...
		conv8(inputPtr, outputConvPtrX, filterPtrX, kernel_width, kernel_height, xSize, ySize, scale, pad);
		conv8(inputPtr, outputConvPtrY, filterPtrY, kernel_width, kernel_height, xSize, ySize, scale, pad);
		for (i = 0; i < xSize; i++) {
			for (j = 0; j < ySize; j++) {
			*(outputConvPtr+i+j*xSize) = (abs(*(outputConvPtrX + i + j * xSize)) + abs(*(outputConvPtrY + i + j * xSize)));
			}
		}
	}
	counter.Stop();
	dCpuTime = counter.GetElapsedTime() / (double)loopCount;
	printf("C Sobel_XY Performance (ms) = %f \n", dCpuTime * 1000.0);

	err = fopen_s(&output_fp, "Sobel_XY_outfile.raw", "wb");
	if (err != 0) {
